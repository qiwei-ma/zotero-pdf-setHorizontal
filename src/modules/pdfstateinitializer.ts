...
export class PDFStateInitializer {
                ...
                notify: (async (
                    ...
                    for (const id of ids) {
                        try {
                            ...
                            if (!item || !item.isAttachment() || !(item as any).isPDFAttachment?.()) continue;

                            ztoolkit.log("[PDFStateInitializer] 检测到 PDF 附件添加，准备设置默认阅读状态", itemID);
    ...
    private static async writeDefaultState(item: Zotero.Item) {
        ...
        await Zotero.File.putContents(stateFile, JSON.stringify(defaultState, null, 2));
